const interopDefault = r => r.default || r || [];
const styles = {
  "node_modules/nuxt/dist/app/entry.js": () => import('./entry-styles.BPw5kTns.mjs').then(interopDefault),
  "pages/blog/[slug].vue": () => import('./_slug_-styles.CeV5hbGX.mjs').then(interopDefault),
  "pages/index.vue": () => import('./index-styles.bR9VHmtZ.mjs').then(interopDefault),
  "pages/blog/[slug].vue?vue&type=style&index=0&scoped=416ca632&lang.css": () => import('./_slug_-styles.CucdKxOH.mjs').then(interopDefault),
  "components/BlogCard.vue": () => import('./BlogCard-styles.CWKOi5nG.mjs').then(interopDefault),
  "pages/index.vue?vue&type=style&index=0&scoped=45257502&lang.css": () => import('./index-styles.chVEWuaS.mjs').then(interopDefault),
  "node_modules/nuxt/dist/app/components/error-500.vue": () => import('./error-500-styles.DH4SuYyS.mjs').then(interopDefault),
  "node_modules/nuxt/dist/app/components/error-404.vue": () => import('./error-404-styles.DYPeZSru.mjs').then(interopDefault),
  "components/BlogCard.vue?vue&type=style&index=0&scoped=e32211e5&lang.css": () => import('./BlogCard-styles.BZ4J5uGL.mjs').then(interopDefault),
  "node_modules/nuxt/dist/app/components/error-500.vue?vue&type=style&index=0&scoped=4b6f0a29&lang.css": () => import('./error-500-styles.D5zYv3n5.mjs').then(interopDefault),
  "node_modules/nuxt/dist/app/components/error-404.vue?vue&type=style&index=0&scoped=06403dcb&lang.css": () => import('./error-404-styles.DFuUZ_1i.mjs').then(interopDefault)
};

export { styles as default };
//# sourceMappingURL=styles.mjs.map
